...
- 集成测试：除单元测试外，必须编写可独立运行的集成测试脚本（位于 tests/ 目录），覆盖典型 Use Case 的完整流程（命令行/API），目的是替代人工端到端验证，提高测试效率
- 优先使用标准库，第三方库需有充足理由
- 单个方法不超过 100 行
...
- `/Users/xmli/me/code/lmd` - TypeScript 的个人知识库和记忆框架。何时参考: hybrid search （keyword + vector）, RRF融合，MMR排序
