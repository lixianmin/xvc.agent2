...
- TDD：测试优先，在完成测试之间，禁止编写任何实现代码。
   - 单元测试：位于 tests/unit目录， 测试覆盖率: 核心包 ≥ 80%
   - 集成测试：位于 tests/integration 目录，覆盖典型 Use Case 的完整流程（命令行/API），以替代人工端到端验证，提高测试效率
   - 每一条用户反馈的bug都应先整理成1~n条失败的测试用例，然后通过跑通测试证明bug已经修复成功
- 优先使用标准库，第三方库需有充足理由
- 单个方法不超过 100 行
